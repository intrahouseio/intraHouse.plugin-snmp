...
var Session = function (target, community, options) {
	...
	this.dgram = dgram.createSocket ({ type: this.transport, reuseAddr: true });
	this.dgram.unref();
...
Session.prototype.get = function (host, oids, responseCb) {
	...
	this.simpleGet (GetRequestPdu, feedCb, pduVarbinds, responseCb, null, host);
...
Session.prototype.getNext = function (host, oids, responseCb) {
	function feedCb (req, message) {
		var pdu = message.pdu;
		var varbinds = [];
	...
	this.simpleGet (GetNextRequestPdu, feedCb, pduVarbinds, responseCb, null, host);
...
Session.prototype.send = function (req, noWait, host) {
	try {
		var me = this;

		var buffer = req.message.toBuffer ();
		this.dgram.send (buffer, 0, buffer.length, req.port, host,
...
Session.prototype.subtree  = function () {
	...
	this.walk (host, oid, maxRepetitions, subtreeCb.bind (me, req), doneCb);
...
function walkCb (req, error, varbinds) {
	...
	if (done)
		...
		this.walk (req.host, oid, req.maxRepetitions, req.feedCb, req.doneCb,
				req.baseOid);
}
...
Session.prototype.walk  = function () {
	...
	if (this.version == Version2c)
		this.getBulk ([oid], 0, maxRepetitions,
				walkCb.bind (me, req), host);
	else
		this.getNext (host, [oid], walkCb.bind (me, req));
